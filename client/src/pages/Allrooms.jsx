        ...
        <div className='flex flex-col-reverse lg:flex-row items-start justify-between pt-28 md:pt-35 px-4 md:px-16 lg:px-24 xl:px-32'>
            <div >
                <div>
                    ...
                    <p className='text-sm md:text-base  text-gray-500/90 mt-2'>Take advantage of our limited time offers and packages to enhance your experience and create unforgetable memories.</p>
                </div>
                {roomsDummyData.map((room) => (
                    <div key={room._id} className='flex flex-col md:flex-row items-start py-10 gap-6 border-b border-gray-300 last:pb-30 last:border-0'>
                        <img onClick={() => {navigate(`/rooms/${room._id}`);scrollTo(0,0)}} className='max-h-65 md:w-1/2 rounded-xl shadow-lg object-cover cursor-pointer' src={room.images[0]} alt="hotel-img" title='View Room Details' />
            ...
            <div className='bg-white w-80 border border-gray-300 text-gray-600 max-lg:mb-8 min-lg:mt-16'>
                        <div className={`flex items-center justify-between px-5 py-2.5 min-lg:border-b border-gray-300 ${openFilter&&"border-b"}`}>
                            ...
                            <span onClick={()=>setOpenfilters(!openFilter)} className=' lg:hidden'>{openFilter?"HIDE":"SHOW"}</span>
